# Загрузка данных о транзакциях из Excel-файлов в DWH
...
def transactions_to_staging(conn, path, logger):
    """
    Ф-ция забирает файлы с данными о транзакциях по заданному паттерну.
    ...
    pattern = r'transactions_[0-9]+.\.txt'

    logger.info(f"""
    Start parsing data: transactions. Path: {path} Pattern: {pattern}
    """)
    ...
    for filename in filenames_list:
        ...
        for datastring in df.values.tolist():
            list_to_insert.append(datastring[0].split(';'))

        logger.info('Created dataframe (10 rows shown:')
        logger.info(f'\n{df.to_string(max_rows=10)}')

        # Запишем датафрейм в слой сырых данных demipt2.gold_stg_transactions_raw
        ...
        # Запись датафрейма в demipt2.gold_stg_transactions_raw
        try:
            query = f"""
                ...
                """
            logger.info('Insert data into demipt2.gold_stg_transactions_raw with query:\n')
            logger.info(query)
            curs.executemany(query, list_to_insert)
            logger.info('Data was inserted into demipt2.gold_stg_transactions_raw!')
        except Exception as e:
            logger.info(f'Data was not inserted into demipt2.gold_stg_transactions_raw! Exception: {e}')
            exit()
        ...
        query = """
                        insert into demipt2.gold_stg_transactions ( 
                            trans_id,
                            trans_date,
                            card_num,
                            oper_type,
        ...
        query = """
                        insert into demipt2.gold_dwh_fact_transactions ( 
                            trans_id,
                            trans_date,
                            card_num,
                            oper_type,
                        ...
                        select 
                            ...
                            oper_result,
                            terminal
                        from demipt2.gold_stg_transactions
                        """
        make_sql_query(conn=conn, query=query, logger=logger)
        ...
        query = """
                        ...
                        set last_update_dt = ( select max(trans_date) from demipt2.gold_stg_transactions )
                        where table_db = 'demipt2' and table_name = 'gold_dwh_fact_transactions' 
                        and ( select max(trans_date) from demipt2.gold_dwh_fact_transactions ) is not null
                        """
        make_sql_query(conn=conn, query=query, logger=logger)
